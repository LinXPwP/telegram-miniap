  ...
  msgs.forEach(m => {
    ...
    const html = `
        ...
        <div class="msg-content">
            ...
            ${seenConfig && m.id === seenConfig.targetId ? `<div class="seen-footer">${seenConfig.text}</div>` : ''}
        </div>`;
    ...
    } else if (!row.innerHTML.includes(content)) {
        row.innerHTML = html; // Update content only if changed
        row.querySelector('.btn-reply-mini')?.addEventListener('click', (e) => { e.stopPropagation(); onReply?.(m); });
        row.querySelector('.msg-reply-preview')?.addEventListener('click', (e) => { e.stopPropagation(); onJumpTo?.(e.currentTarget.dataset.jumpId); });
    }
  });
...
function initUserApp() {
  ...
  const selTicket = (id) => {
    ...
    // Seen logic
    let seen = null;
    const lastUserM = [...t.messages].reverse().find(m=>m.from==='user'&&!m.deleted);
    if(lastUserM && t.last_read_admin && t.last_read_admin >= lastUserM.id) seen = { targetId: lastUserM.id, text: `Văzut ${timeAgo(t.last_read_admin_at)}` };
  ...
  const sendMsg = async () => {
    ...
    try {
        ...
        if(res.ok && res.ticket) {
            ...
            renderTickets(); renderDiscordMessages(res.ticket.messages, {container: els.msgs, ticket:res.ticket}); smartScrollToBottom(els.msgs, true);
     ...
     if(res.ok && res.tickets) {
         ...
         if(STATE.selTicketId) {
             ...
             if(t) {
                ...
                renderDiscordMessages(t.messages, {container: els.msgs, ticket:t, canReply:t.status==="open", onReply:setReply, seenConfig:null /*recalc seen handled in selTicket or simpler UI update*/ });
                updateChatUI(t);
             }
         }
         renderTickets();
