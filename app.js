// app.js – Versiune User FINALĂ (Fixed Anti-Flicker, Reply, Style, Images)
...
function createSmartPoll(fetchFn, isEnabledFn, options = {}) {
  ...
  let timeoutId = null, active = false, currentInterval = minInterval;
  ...
  async function tick() {
    ...
    try {
      ...
      if (!active) return;
      if (data !== undefined) {
         currentInterval = minInterval; // Reset on success
      }
    } catch (e) { 
  ...
  return { start() { if (active) return; active = true; currentInterval = minInterval; tick(); }, stop() { active = false; if (timeoutId) clearTimeout(timeoutId); timeoutId = null; }, bumpFast() { if (!active) return; currentInterval = minInterval; schedule(currentInterval); } };
}
