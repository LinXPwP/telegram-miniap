...
const timeAgo = (ts) => {
  ...
  return h < 24 ? `${h}h` : `${Math.floor(h / 24)}z`;
};
...
const getImageUrl = (s) => (s && s.trim() ? s : null);
...
function getSeenConfig(t) {
  ...
  if (lastReadAdmin >= lastUserMsgId) {
    return {
      ...
      text: `Văzut ${
        t.last_read_admin_at ? timeAgo(t.last_read_admin_at) : ""
      }`,
    };
  }
  return null;
...
function calculateUserUnread(ticket) {
  ...
  const count = ticket.messages.filter(
    (m) => m.from === "admin" && Number(m.id) > lastReadId
  ).length;
...
function renderDiscordMessages(
  msgs,
  { container, canReply, onReply, onJumpTo, seenConfig }
) {
  if (!container) return;
  ...
  msgs.forEach((m) => {
    ...
    const replyHtml =
      ...
      <div class="msg-reply-preview" data-jump-id="${msgMap[m.reply_to].id}">
        <strong style="margin-right:5px;">${
          msgMap[m.reply_to].sender || "User"
        }</strong>
        <span>${(msgMap[m.reply_to].text || "").slice(0, 50)}...</span>
      </div>`
        : "";
    ...
    const btns =
      canReply && !m.deleted
        ? `<button class="btn-reply-mini" title="Răspunde">↩ Reply</button>`
        : "";
        ...
        <div class="msg-header-line">
          <div class="msg-meta-group">
            <span class="msg-username ${
              m.from === "admin" ? "msg-username--admin" : ""
            }">${sender}</span>
    ...
    if (!row) {
      ...
      row
        .querySelector(".msg-reply-preview")
        ?.addEventListener("click", (e) => {
          e.stopPropagation();
          const targetId = e.currentTarget.dataset.jumpId;
          onJumpTo && onJumpTo(targetId);
        });
    } else {
      ...
      if (textEl && textEl.textContent !== content) {
        ...
        if (m.deleted) {
          textEl.className = "msg-text msg-text--deleted";
        }
      }
    }
  ...
  Array.from(container.children).forEach((c) => {
    if (c.dataset.messageId && !renderedIds.has(c.dataset.messageId)) {
      c.remove();
    }
  });
...
function initUserApp() {
  ...
  const els = {
    ...
    // aici era bug-ul cu $ pe class – folosim querySelector
    inputCont: document.querySelector(".chat-input"),
  ...
  els.creditsM?.addEventListener("click", (e) => {
    if (e.target === els.creditsM) hide(els.creditsM);
  });
  ...
  modeBar.innerHTML = `
    <span class="chat-mode-text"></span>
  ...
  const setReply = (msg) => {
    userMode = {
      type: "reply",
      msgId: msg.id,
      txt: (msg.text || "").slice(0, 50),
      sender: msg.sender || "User",
    };
    modeBar.querySelector(
      ".chat-mode-text"
    ).textContent = `Răspunzi lui ${userMode.sender}: "${userMode.txt}..."`;
    show(modeBar);
    els.input?.focus();
  };
  ...
  const updateChatUI = (t) => {
    ...
    if (!t) {
      ...
      els.input.placeholder = "Alege un tichet...";
      hide(modeBar);
      hide(els.closeT);
      hide(els.reopenT);
      els.tTitle.textContent = "Niciun tichet";
      return;
    }
  ...
  const renderHeader = () => {
    ...
    els.userLine.innerHTML = `Utilizator: <b>${
      STATE.user.username ? "@" + STATE.user.username : "ID " + STATE.user.id
    }</b>`;
  };
  ...
  const renderCats = (shop) => {
    ...
    shop?.categories?.forEach((cat) => {
      ...
      d.onclick = () => {
        ...
        hide(els.title);
        show(els.backBtn);
        els.backBtn.querySelector(".back-btn-text").textContent = cat.name;
  ...
  const selVar = (t, btn) => {
    SELECTED_VARIANT = t;
    Array.from(els.mTypesGrid.children).forEach((c) =>
      c.classList.remove("active")
    );
    btn.classList.add("active");
    els.mPrice.textContent = `${t.price} CRD`;
    els.mDesc.textContent = `${
  ...
  const openModal = (p) => {
    ...
    els.mBuy.textContent = "Cumpără acum";
    ...
    if (img) {
      ...
      hide(els.mImg);
      show(els.mPlace, "flex");
    }

    if (p.types?.length) {
      show(els.mTypes, "block");
      els.mTypesGrid.innerHTML = "";
      p.types
        .slice()
      ...
      hide(els.mTypes);
      els.mPrice.textContent = `${p.price} CRD`;
      els.mDesc.textContent = p.description || "Fără descriere.";
    }
  ...
  els.mBuy.onclick = async () => {
    ...
    if (SELECTED_PRODUCT.types?.length && !SELECTED_VARIANT) {
      els.mStatus.textContent = "Selectează o variantă!";
    ...
    els.mBuy.textContent = "Se procesează...";
    ...
    try {
      ...
      if (!res.ok) {
        STATE.buying = false;
        els.mBuy.disabled = false;
        els.mBuy.textContent = "Încearcă din nou";
        els.mStatus.className = "status-message status-error";

        if (res.error === "not_enough_credits") {
          els.mStatus.innerHTML =
            'Fonduri insuficiente! <span style="text-decoration:underline;cursor:pointer;font-weight:bold" onclick="document.getElementById(\'creditsModal\').style.display=\'flex\'">Încarcă</span>';
        } else {
          els.mStatus.textContent = "Eroare: " + res.error;
        }
      } else {
        ...
        els.mStatus.textContent = "Succes!";
        setTimeout(() => {
          closeModal();
          setTab(false);
          STATE.buying = false;
      ...
      STATE.buying = false;
      els.mBuy.disabled = false;
      els.mStatus.textContent = "Eroare rețea.";
    }
  };
  ...
  const renderTickets = () => {
    ...
    if (!STATE.tickets.length) {
      els.chatList.innerHTML =
        '<div style="padding:20px;text-align:center;color:#555;">Nu ai tichete.</div>';
      return;
    }
    ...
    STATE.tickets
      ...
      .forEach((t) => {
        ...
        item.className =
          "chat-item " + (t.id === STATE.selTicketId ? "active" : "");
        ...
        item.innerHTML = `
          <div class="chat-item-header-row">
            ...
            <div>
              ...
              <span class="ticket-status-pill ${t.status}">${
          t.status
        }</span>
            </div>
          </div>
  ...
  const selTicket = (id) => {
    ...
    renderDiscordMessages(t.messages, {
      ...
      onReply: setReply,
      onJumpTo: (mid) => {
        const el = els.msgs.querySelector(
          `.msg-row[data-message-id="${mid}"]`
        );
        if (el) {
          el.classList.add("msg-row--highlight");
          el.scrollIntoView({ behavior: "smooth", block: "center" });
          setTimeout(
            () => el.classList.remove("msg-row--highlight"),
            1200
          );
        }
      },
  ...
  els.menu?.addEventListener("click", () => {
    els.ticketsTab.classList.toggle("tickets-drawer-open");
  });
    ...
    els.okConf.onclick = async () => {
      ...
      if (res.ok) {
        const idx = STATE.tickets.findIndex(
          (x) => x.id === STATE.selTicketId
        );
        if (idx >= 0) {
          STATE.tickets[idx] =
            res.ticket || { ...STATE.tickets[idx], status: "closed" };
          renderTickets();
          updateChatUI(STATE.tickets[idx]);
        }
      }
    };
    els.canConf.onclick = () => hide(els.confirm);
    els.confirm.onclick = (e) => {
      if (e.target === els.confirm) hide(els.confirm);
    };
  });

  els.reopenT?.addEventListener("click", async () => {
    if (!STATE.selTicketId) return;
    els.reopenT.textContent = "...";
    els.reopenT.disabled = true;
    const res = await apiCall("user_reopen_ticket", {
      ticket_id: STATE.selTicketId,
    });
    els.reopenT.textContent = "Redeschide";
    els.reopenT.disabled = false;
    if (res.ok && res.ticket) {
      const idx = STATE.tickets.findIndex(
        (x) => x.id === STATE.selTicketId
      );
  ...
  (async () => {
    ...
    if (res.ok) {
      ...
      els.userLine.innerHTML = `<span style="color:red">Eroare: ${
        res.error || "Auth"
      }</span>`;
      show(els.userLine, "block");
    }
  })();
}
